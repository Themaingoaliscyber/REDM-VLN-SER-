            ...
            -- Shop Open
                ...
                if distance <= shopCfg.shop.distance then
                    ...
                    if Citizen.InvokeNative(0xC92AC953F0A982AE, BuyPrompt) then -- PromptHasStandardModeCompleted
                        ...
                        TriggerServerEvent('vln-guarma:BuyTicket', shop)
...
function CheckPlayerJob(shop)
    HasJob = false
    local result = Core.Callback.TriggerAwait('vln-guarma:CheckJob', shop)
    if result then
        HasJob = true
    end
end
...
function CheckPlayerTicket(shop)
    local canTravel = Core.Callback.TriggerAwait('vln-guarma:CheckTicket')
    if canTravel then

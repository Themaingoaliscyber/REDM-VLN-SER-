...
RegisterCommand("editnews", function(source, args)
    TriggerServerEvent('vln_news:getjob')
    Wait(500)
   ...
   MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
	...
	function(data, menu)
        ...
        if(data.current.value == "confirm") then
            MenuData.CloseAll() 
            TriggerServerEvent("vln_news:savenews",news)
        end
		if(data.current.value == "front") then
            MenuData.CloseAll() 
            TriggerEvent("vln_inputs:sendinputs", "Confirm", "Link", function(cb)
        ...
        if(data.current.value == "edit") then
            MenuData.CloseAll() 
            TriggerEvent("vln_inputs:sendinputs", "Confirm", "Link, Leave Empty to Remove", function(cb)
                local descx =     tostring(cb)
                if descx ~= nil and descx ~= "" then
                    if string.find(descx, "http") then
...
RegisterNetEvent('vln_news:recjob')
AddEventHandler('vln_news:recjob', function(x,y)
	job = x 
    news = y
end)
...
RegisterNetEvent('vln_news:open')
AddEventHandler('vln_news:open', function()
    TriggerServerEvent('vln_news:getjob')
    Wait(500)
	OpenUI()
end)

...
BccUtils = exports['vln-utils'].initiate()
Progressbar = exports["vln-feather-progressbar"]:initiate()
local MiniGame = exports['vln-minigames'].initiate()
...
local useMudBucketPrompt = promptGroup:RegisterPrompt(_TU('promptMudBucket'), Config.keys.E, 1, 1, true, 'hold',
    { timedeventhash = "MEDIUM_TIMED_EVENT" })
local useWaterBucketPrompt = promptGroup:RegisterPrompt(_TU('promptWaterBucket'), Config.keys.R, 1, 1, true, 'hold',
    { timedeventhash = "MEDIUM_TIMED_EVENT" })
local useGoldPanPrompt = promptGroup:RegisterPrompt(_TU('promptPan'), Config.keys.G, 1, 1, true, 'hold',
    { timedeventhash = "MEDIUM_TIMED_EVENT" })
local removeTablePrompt = promptGroup:RegisterPrompt(_TU('promptPickUp'), Config.keys.F, 1, 1, true, 'hold',
    { timedeventhash = "MEDIUM_TIMED_EVENT" })
...
function IsNearWater()
    ...
    if not isInAllowedZone then
        VORPcore.NotifyObjective(_TU('noWater'),4000)
        return
    end
...
RegisterNetEvent('bcc-goldpanning:waterUsedSuccess', function()
    local playerPed = PlayerPedId()
    Citizen.InvokeNative(0x524B54361229154F, playerPed, GetHashKey('WORLD_HUMAN_BUCKET_POUR_LOW'), -1, true, 0, -1, false)
    Progressbar.start(_TU('pouringWater'), Config.bucketingTime, function()
        ClearPedTasks(playerPed, true, true)
        Citizen.InvokeNative(0xFCCC886EDE3C63EC, playerPed, 2, true)
        Wait(100)
    end, 'linear', 'rgba(255, 255, 255, 0.8)', '20vw', 'rgba(255, 255, 255, 0.1)', 'rgba(211, 211, 211, 0.5)')
...
AddEventHandler('bcc-goldpanning:useEmptyMudBucket', function()
    if IsNearWater() then
        ...
        Progressbar.start(_TU('collectingMud'), Config.bucketingTime, function()
            ClearPedTasks(playerPed, true, true)
            Citizen.InvokeNative(0xFCCC886EDE3C63EC, playerPed, 2, true)
            Wait(100)
...
AddEventHandler('bcc-goldpanning:useWaterBucket', function()
    if IsNearWater() then
        local playerPed = PlayerPedId()
        Citizen.InvokeNative(0x524B54361229154F, playerPed, GetHashKey('WORLD_HUMAN_BUCKET_FILL'), -1, true, 0, -1, false)
        Progressbar.start(_TU('collectingWater'), Config.bucketingTime, function()
            ClearPedTasks(playerPed, true, true)
            Citizen.InvokeNative(0xFCCC886EDE3C63EC, playerPed, 2, true)
            Wait(100)
...
AddEventHandler('bcc-goldpanning:canCarryResponse', function(canCarry)
    if canCarry then
        ...
        VORPcore.NotifyObjective(_TU('propFull'), 4000)
    end
end)
...
AddEventHandler('bcc-goldpanning:placeProp', function(propName)
    ...
    while placing do
        ...
        if PromptHasHoldModeCompleted(BuildPrompt) then
            ...
            Progressbar.start(_TU('buildingTable'), Config.washBuildTime, function()
            end, 'linear', 'rgba(255, 255, 255, 0.8)', '20vw', 'rgba(255, 255, 255, 0.1)', 'rgba(211, 211, 211, 0.5)')
...
function PlayAnim(animDict, animName, time, raking, loopUntilTimeOver) --function to play an animation
    ...
    if raking then
        ...
        Progressbar.start(_TU('siftingGold'), time, function()
            Wait(5)

...
Citizen.CreateThread( function()
	...
	repeat
		if WarMenu.IsMenuOpened('fence2') then
			for i = 1, #Config.FenceShop do
				if WarMenu.Button(Config.FenceShop[i]['Text'], Config.FenceShop[i]['SubText'], Config.FenceShop[i]['Desc']) then
					TriggerServerEvent('bar:comprar', Config.FenceShop[i]['Param'])
				end
			end
			WarMenu.Display()
		end
...
AddEventHandler('UI:NotificaCompra', function( _message )
	TriggerEvent("vorp:TipRight", _message, 1000)
end)

Citizen.CreateThread(function()
	while true do

		local IsZone, IdZone = IsNearZone(Config.Coords2)      
       
        if IsZone then 
			DisplayHelp(Config.Blacktext, 0.50, 0.95, 0.6, 0.6, true, 255, 255, 255, 255, true, 10000)
...
Citizen.CreateThread(function()
	for k, v in pairs(Config.Blips) do
		...
		Citizen.InvokeNative(0x9CB1A1623062F402, blip, 'BlackMarket SELL')
	end  
end)